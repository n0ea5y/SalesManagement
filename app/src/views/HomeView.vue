...
onMounted(async () => {
  await getDailySalesMaster(day.value);
...
const day = ref(new Date().toLocaleDateString("ja-JP", { year: "numeric", month: "2-digit", day: "2-digit" }).replaceAll('/', '-'))
const addMode = ref(true);
const dialog = shallowRef(false)

watch(() => day.value, (newVal) => {
...
const submit = async () => {
  ...
  await addDoc(collection(db, `daily_sales_${day.value}`), {
  ...
  dialog.value = false;
  insertToast();
  getDailySalesMaster(day.value);
}

const update = async () => {
  await updateDoc(doc(db, `daily_sales_${day.value}`, todaySale.value.id), {
    'in_time': todaySale.value.in_time,
    'table_number': todaySale.value.table_number,
    'media_name': todaySale.value.media_name,
    'staff_name': todaySale.value.staff_name ?? null,
...
const getDailySalesMaster = async (day) => {
  const querySnapshot = await getDocs(collection(db, `daily_sales_${day}`));
...
<template>
  <AuthLayout>
    ...
    <div class="w-1/2 mx-auto">
      <SmText type="date" bordernone v-model="day"></SmText>
    </div>
